...
class ConfigsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create config" do
    assert_difference('Config.count') do
      post configs_url, params: { config: { adresse: @config.adresse, nom: @config.nom, post: @config.post } }
    end
  ...
  test "should update config" do
    patch config_url(@config), params: { config: { adresse: @config.adresse, nom: @config.nom, post: @config.post } }
